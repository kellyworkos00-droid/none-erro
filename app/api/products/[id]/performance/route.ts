...
export async function GET(
  ...
  try {
    ...
    const totalSold = orderItems.reduce((sum: number, item: { quantity: number }) => sum + item.quantity, 0);
    const totalRevenue = orderItems.reduce((sum: number, item: { totalPrice: number }) => sum + item.totalPrice, 0);
    const totalOrders = new Set(orderItems.map((item: { posOrderId: string }) => item.posOrderId)).size;
    ...
    const recentOrderItems = orderItems.filter(
      (item: { createdAt: Date }) => new Date(item.createdAt) >= thirtyDaysAgo
    );

    // Group by date
    const salesByDate = new Map<string, { quantity: number; revenue: number }>();
    recentOrderItems.forEach((item: { createdAt: Date; quantity: number; totalPrice: number }) => {
    ...
    for (const p of allProducts) {
      ...
      const revenue = items.reduce((sum: number, item: { totalPrice: number }) => sum + item.totalPrice, 0);
