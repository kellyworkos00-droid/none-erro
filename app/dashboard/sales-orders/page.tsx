...
export default function SalesOrdersPage() {
  ...
  const updateItem = (index: number, updates: Partial<SalesOrderItemForm>) => {
    setItems((prev) =>
  ...
  const handleAction = async (
    orderId: string,
    action: 'SUBMIT' | 'APPROVE' | 'DELIVER' | 'INVOICE' | 'CANCEL'
  ) => {
    try {
      const token = localStorage.getItem('token');
        ...
        <div className="card-body">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600">Track approvals, deliveries, and invoicing.</p>
            </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {loading ? (
            ...
            <div className="space-y-3">
              {orders.map((order) => (
                <div key={order.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-3">
                  <div>
