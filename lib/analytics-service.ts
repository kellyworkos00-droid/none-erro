  ...
  try {
    ...
    for (let dayOffset = 0; dayOffset <= daysDuration; dayOffset++) {
      ...
      const inflows = payments.reduce(
        (sum: Decimal, p: { amount: number }) => sum.plus(new Decimal(String(p.amount))),
        new Decimal(0)
      );
...
async function getTotalAssets(): Promise<Decimal> {
  ...
  return accounts.reduce(
    (sum: Decimal, account: { currentBalance: number | null }) => sum.plus(new Decimal(String(account.currentBalance || 0))),
    new Decimal(0)
  );
}
